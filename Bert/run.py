...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_output_dir in eval_outputs_dirs:
        ...
        result = {"mcc" : matthews_corrcoef(out_label_ids, preds)}
        accu = {'accu' : accuracy_score(out_label_ids, preds)}
        f1s = {'f1_score' : f1_score(out_label_ids, preds)}
        
        js = json.dumps(results)   
        file = open('result_eval_1.txt', 'w')  
        file.write(js)  
        file.close()  
        results.update(accu)
        results.update(f1s)
...
def init_args(parser):
    ...
    ALL_MODELS = ['bert', 'bert_cnn', 'bert_rnn','bert_rcnn','bert_dpcnn'] 
    parser.add_argument(
        "--model",
        default=None,
        type=str,
        required=True,
        help="The name of model selected in the list: " + ", ".join(ALL_MODELS),
    )
    parser.add_argument(
        "--data_dir",
        default=None,
...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        config.tokenizer.from_pretrained(args.output_dir)
        model.to(args.device)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, config.tokenizer, prefix=prefix)
            result = dict((k + "_{}".format(global_step), v) for k, v in result.items())
            results.update(result)
        logger.info(results)

        js = json.dumps(results)   
        file = open('result_eval.txt', 'w')  
        file.write(js)  
        file.close()  
    print(results)
    return results
